...
- **Navigation**: Use the 'Summary Table' to jump to specific files.
- **Structure**: Files are encapsulated in `<file>` tags with a `path` attribute.
- **Content**: Actual file data is wrapped in `<content>` tags.
- **Resilience**: Code blocks use dynamic backtick depth to prevent nesting breakage.
</system_instructions>"""

# XML-style tags for better LLM anchoring
TAG_PROJECT_STRUCTURE = "project_structure"
TAG_FILE_REPOSITORY = "file_repository"
TAG_FILE = "file"
TAG_CONTENT = "content"
